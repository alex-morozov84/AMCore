...
- **Top route progress bar.** A first-party (no dependency),
  ...
  clicks start it through a new `RouteProgressLink`
  (`shared/ui/route-progress-link.tsx`), using `next/link`'s `onNavigate` so
  ...
  for the new `pnpm init:project --route-progress=disabled` flag, which only
...
- **Transactional and manifest-backed downstream initialization.** `pnpm init:brand` and
  ...
  independent CSP security entrypoint. `init:brand` now uses the same
  desired-state, ownership, structural-operation, and single-transaction
  pipeline. Its logo/icon inputs are snapshotted once as validated bytes plus
  mode before confirmation, and the former brand planners, step materializer,
  write closures, and compatibility executor have been removed. The public
  flags, prompts, output, repeatability, and generated bytes remain unchanged.
...
- **Server-rendered graceful degradation** (ADR-079, P1 item 7).
  `shared/api/server/` — Server Components fetch `apps/api` directly (never
  looping back through this app's own `/api/*` routes), returning a closed
  `DataOutcome<T>` (`success`/`not-found`/`unavailable`) instead of
  ...
  silently (logged once), primary content renders
  `shared/ui/primary-unavailable-fallback.tsx` (also logged once) instead
  of ever faking an empty state, with a locale-aware `router.refresh()`
  retry control. `shared/ui/section-error-boundary.tsx` (`catchError`) is
  the safety net for genuinely unexpected exceptions only — the only case
  `instrumentation.ts`'s `onRequestError` still needs to handle. A new
  minimal `shared/lib/server-logger/` (Pino, bounded/redacted fields,
  volume-suppressed) gives `apps/web` its own structured logging for the
  first time, closing a real gap between
...
[unreleased]: https://github.com/alex-morozov84/AMCore/compare/v0.9.0...HEAD
