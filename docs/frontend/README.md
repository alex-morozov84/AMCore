...
| [API consumption](./api-consumption.md)                         | The hooks that consume media/notifications/AI through the BFF, the `EventSource`-based realtime pattern shared by notifications and AI, and the opt-in client-IP relay that lets `apps/api`'s rate limiter tell BFF visitors apart |
