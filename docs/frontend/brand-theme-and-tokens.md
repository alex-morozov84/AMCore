...
`--route-progress=disabled` only flips the top route-progress bar's source
flag to its disabled default and is **non-destructive** — no file is moved
...
seam. Missing, stale, ambiguous, and duplicate declarations fail during
planning, before the filesystem transaction starts. Shared helpers survive
