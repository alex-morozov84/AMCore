...
| `pagination.tsx`                     | shadcn (Base UI), adapted                   | Link-based pagination uses `RouteProgressLink` for internal navigation and requires caller-provided, localized previous/next labels. The Audit console composes cursor-based previous/older links without inventing page numbers.                                                                                                                                                                                                         |
| `calendar.tsx`, `popover.tsx`        | shadcn (Base UI), hand-ported               | Added for the console date range control from a scratch worktree. Calendar uses React DayPicker; Popover uses Base UI. Existing customized `button.tsx` was preserved.                                                                                                                                                                                                                                                                    |
| `date-time-range-picker.tsx`         | AMCore-authored composition                 | Reusable calendar range plus start/end time fields. The caller supplies locale, time zone, labels, values, and validation; audit-specific limits stay in the Audit page.                                                                                                                                                                                                                                                                  |
