...
> **Transitional rows.** The two `(transitional)` rows reflect the current allowlist after Stage 2. Stage 4 of the Organizations/Admin review (see `ai/ORGANIZATIONS_ADMIN_REVIEW.md`) may add per-handler narrowings on the org members and roles surfaces — but it will not expand API-key access. The handler-level `@CheckPolicies` decorators remain the actual authorization gate within each accepted route. The `/organizations` rows are stable post-Stage 2.
