...
> **Transitional row.** The remaining `(transitional)` row reflects the current allowlist mid-Stage 4. The `/organizations/:id/roles/**` surface is expected to gain stricter org-context assertions in the next pass (OA-06 in `ai/ORGANIZATIONS_ADMIN_REVIEW.md`) — but the API-key opt-in stays. The handler-level `@CheckPolicies` decorators remain the actual authorization gate within each accepted route.
