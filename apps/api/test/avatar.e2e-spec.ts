...
// A valid 1x1 RGBA PNG generated by libvips (sharp) itself. The previous
// hand-minified blob decoded on libpng < 1.6.50 but the pixel read now fails
// with `vipspng: libpng read error` under the stricter libpng bundled in
// libvips 8.18 (sharp 0.35). Metadata (header-only) still parsed, so the break
// surfaced only in the full decode/resize path. Regenerate via sharp if it ever
// regresses again — never hand-mint a PNG here.
...
describe('Avatar storage (e2e)', () => {
  ...
  it('wires the global guard to the Redis-backed rate limiter (ADR-039/ADR-073)', () => {
    // Guards against a config that builds the limiter but fails to wire it
    // into RateLimitGuard, silently falling back to in-memory (process-local)
    // limits.
    expect(context.throttlerStorage).toBeInstanceOf(GcraRedisLimiter)
  })
})
