...
/**
 * The subset of an Express/Node response the SSE writer needs. Kept minimal so the connection is
 * unit-testable with a fake — no HTTP server required.
 */
...
/**
 * One server-side AI run-status SSE stream (Track C — ADR-054, Arc C.5; ADR-053 pattern). Owns the
 * write path and its backpressure: data frames are written directly while the socket drains and
 * buffered (bounded by `queueDepth`) while it does not; overflowing the buffer **destroys** the
 * socket (graceful `end()` could block behind the same stuck socket) so the client reconnects and
 * resyncs. Heartbeats are *skipped* while backpressured, never queued. `close()` is idempotent and
 * fires `onClose` once. Every `data:` frame is a content-free run-status hint (no `id:`/`event:`).
 */
export class AiRunStreamConnection {
  ...
  /** Send a disposable run-status hint as an SSE `data:` frame (no `id:`/`event:`; ADR-053). */
  sendData(event: AiRunSseEvent): void {
    if (this.closed) return
