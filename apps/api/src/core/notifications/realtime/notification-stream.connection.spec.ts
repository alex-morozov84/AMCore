...
const tracked: NotificationStreamConnection[] = []
function connect(res: StreamWritable, onClose: (r: StreamCloseReason) => void, queueDepth = 16) {
  const conn = new NotificationStreamConnection(res, 'user-1', {
    queueDepth,
    lifetimeMs: 60000,
    onClose,
  })
...
describe('NotificationStreamConnection', () => {
  afterEach(() => {
  ...
  it('destroys the socket and reports overflow once the queue is full', () => {
    ...
    // The drain listener registered when backpressure tripped must be removed.
  ...
  it('closes on the lifetime timer with reason expired', () => {
    ...
    try {
      const f = fakeRes()
      const onClose = jest.fn()
      const conn = new NotificationStreamConnection(f.res, 'user-1', {
        queueDepth: 16,
        lifetimeMs: 1000,
        onClose,
      })
