...
export function CopyConsoleId({
  ...
  useEffect(() => {
    ...
    window.addEventListener('console-id-copied', resetOther)
    return () => window.removeEventListener('console-id-copied', resetOther)
  }, [buttonId])
  return (
    <button
      type="button"
      onClick={async () => {
        try {
          ...
          window.dispatchEvent(new CustomEvent('console-id-copied', { detail: buttonId }))
          setState('copied')
        } catch {
          setState('failed')
        }
