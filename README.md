...
readiness/version/process-role, a read-only **Users** inventory, and a read-only
**Organizations** inventory, plus path/host topology, isolated host sessions,
downstream scaffolding, deployment guidance, and a safe extension recipe. It is
not a product backoffice or a catalog/content admin UI.
...
| **Operations**    | ✅ Shipped      | Health, Prometheus metrics + an optional Grafana/Alertmanager monitoring harness with shipped alert rules and per-category runbooks, a CI contract verifying the metric↔alert↔dashboard↔runbook chain stays correct (static + live checks against a real stack), audit log, idempotency, webhooks, TLS/reverse-proxy (incl. an optional bundled Caddy edge profile), Redis-backed GCRA rate limiting with burst tolerance, verified BFF client-identity relay, Postgres backup/restore + restore-drill rehearsal, production database role separation, secret rotation runbook, a build-once/promote-by-digest production deploy profile, a platform decision matrix (Kubernetes, Cloud Run, Fly, Render, Railway, Vercel), CI/security automation |
...
| **Operations Console**   | ✅ Shipped      | Optional `SUPER_ADMIN` control plane with Overview, read-only Users/Organizations, path/host topology, isolated host sessions, scaffolding and extension guidance                           |
...
| Add frontend search                 | [`docs/frontend/architecture-and-conventions.md#recipe-add-search`](docs/frontend/architecture-and-conventions.md#recipe-add-search) — local controlled filtering and URL-backed debounced discovery                                                                                                                                                                                                                                                                                                                                                             |
...
| Operations Console                  | [`docs/operations-console/`](docs/operations-console/README.md) — using Overview, read-only Users and Organizations; topology, session security, scaffolding, deployment, verification, and extension contract                                                                                                                                                                                                                                                                                                                                                   |
