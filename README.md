...
| **Operations**    | ✅ Shipped      | Health, observability, audit log, idempotency, webhooks, TLS/reverse-proxy (incl. an optional bundled Caddy edge profile), Redis-backed rate limiting with a verified BFF client-identity relay, Postgres backup/restore, CI/security automation |
...
| Rate limiting / BFF client identity | [`docs/operations/deployment.md`](docs/operations/deployment.md#bff-client-ip-relay-appsweb--appsapi--a-separate-contract-from-trust_proxy) — Redis-backed global throttler, and the opt-in verified relay that lets it tell BFF visitors apart without trusting a spoofable header |
