...
- **AI Capability Layer** _(foundational)_ — a provider-agnostic AI control plane on its own `ai` Postgres schema: a DB-backed admin-manageable catalog (Anthropic/Claude default + OpenAI/OpenRouter/Yandex/OpenAI-compatible/mock) and a `ModelGateway` over the Vercel AI SDK for credential-gated text + capability-gated structured output, with a usage/cost ledger and content-free metrics; plus the persistence + shared contracts for conversations, the self-hosted tool loop, human takeover, and multimodal artifacts. The gateway is not yet exposed over HTTP — the run worker/API, guardrails, tools, takeover, and multimodal routing land in later arcs (see [`docs/ai/`](docs/ai/README.md))
