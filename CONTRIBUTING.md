...
| `pnpm init:project`                                               | Apply downstream project choices: locale mode, Storybook, route-progress default, and Operations Console topology                                                                                                                                                                                   |
...
| `pnpm measure:scaffold`                                           | Opt-in scaffolding baseline measurement/inventory (maintainer tool) — running the actual baseline is manual, never part of any CI gate; its own unit/regression tests (`pnpm test:scripts:fast`) are required, see [`docs/operations/ci-security.md`](docs/operations/ci-security.md#current-gates) |
...
`--admin-console=disabled|path|host` is another one-time choice. Its optional
